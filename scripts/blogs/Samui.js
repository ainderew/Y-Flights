...
const submitComment = async (event) => {
  ...
  const endpoint = `${endpointURL}comments/SamuiPostComment`;
...
const renderComments = async () => {
  repliesArrayCtr = []
  const endpoint = `${endpointURL}getComments/SamuiComments`;
  await fetch(endpoint, {
    method: "GET",
    mode: "cors",
  })
...
const getReplies =  async (repliesArray,indexOfComment) =>{
  ...
  console.log(repliesArrayCtr[indexOfComment])
    ...
    repliesArray.forEach(async (el,index) => {
      ...
      const endpoint = `${endpointURL}getComments/SamuiReplies` ;
      await fetch(endpoint,{
      mode: "cors",
      method: "POST",
      headers: {
...
const submitReply = async (event,commentId,repliedToName) =>{
  ...
  const endpoint = `${endpointURL}comments/SamuiPostReply`;
