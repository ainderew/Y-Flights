...
const submitComment = async (event) => {
  ...
  const endpoint = `${endpointURL}comments/SrilankaPostComment`;
...
const renderComments = async () => {
  repliesArrayCtr = []
  const endpoint = `${endpointURL}getComments/SrilankaComments`;
  await fetch(endpoint, {
    method: "GET",
    mode: "cors",
  })
...
const getReplies =  async (repliesArray,indexOfComment) =>{
  ...
  console.log(repliesArrayCtr[indexOfComment])
    ...
    repliesArray.forEach(async (el,index) => {
      ...
      const endpoint = `${endpointURL}getComments/SrilankaReplies` ;
      await fetch(endpoint,{
      mode: "cors",
      method: "POST",
      headers: {
...
const submitReply = async (event,commentId,repliedToName) =>{
  ...
  const endpoint = `${endpointURL}comments/SrilankaPostReply`;
