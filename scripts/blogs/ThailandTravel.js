...
const submitComment = async (event) => {
  ...
  const endpoint = `${endpointURL}comments/ThailandTravelPostComment`;
...
const renderComments = async () => {
  repliesArrayCtr = []
  const endpoint = `${endpointURL}getComments/ThailandTravelComments`;
  await fetch(endpoint, {
    method: "GET",
    mode: "cors",
  })
...
const getReplies =  async (repliesArray,indexOfComment) =>{
  ...
  console.log(repliesArrayCtr[indexOfComment])
    ...
    repliesArray.forEach(async (el,index) => {
      ...
      const endpoint = `${endpointURL}getComments/ThailandTravelReplies` ;
      await fetch(endpoint,{
      mode: "cors",
      method: "POST",
      headers: {
...
const submitReply = async (event,commentId,repliedToName) =>{
  ...
  const endpoint = `${endpointURL}comments/ThailandTravelPostReply`;
