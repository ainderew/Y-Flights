...
const submitComment = async (event) => {
  ...
  const endpoint = `${endpointURL}comments/BaliPostComment`;
...
const renderComments = async () => {
  repliesArrayCtr = []
  const endpoint = `${endpointURL}getComments/BaliComments`;
  await fetch(endpoint, {
    method: "GET",
    mode: "cors",
  })
...
const getReplies =  async (repliesArray,indexOfComment) =>{
  ...
  console.log(repliesArrayCtr[indexOfComment])
    ...
    repliesArray.forEach(async (el,index) => {
      ...
      const endpoint = `${endpointURL}getComments/BaliReplies` ;
      await fetch(endpoint,{
      mode: "cors",
      method: "POST",
      headers: {
...
const submitReply = async (event,commentId,repliedToName) =>{
  ...
  const endpoint = `${endpointURL}comments/BaliPostReply`;
